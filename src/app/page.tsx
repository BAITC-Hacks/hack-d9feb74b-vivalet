...
import { ArrowRight, FileText, History, Layers3, ShieldCheck, Sparkles, UploadCloud } from "lucide-react";
...
export default function Home() {
  ...
  async function demo() {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Ошибка демо."); }
    finally { setBusy(false); }
  }
  ...
  return <main className="site-shell">
    <header className="topbar"><div className="brand"><span className="brand-icon"><Layers3 size={20}/></span><span>VIVALET <small>AI Organization Auditor</small></span></div><div style={{display:"flex",gap:16,alignItems:"center"}}><Link href="/history" style={{display:"flex",alignItems:"center",gap:6,color:"#526f90",fontSize:13,fontWeight:700,textDecoration:"none"}}><History size={15}/>История</Link><span className="top-note"><ShieldCheck size={16}/> Анализ с проверяемыми источниками</span></div></header>
