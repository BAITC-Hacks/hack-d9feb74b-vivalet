...
import { z } from "zod";
import { criticSchema, decisionSchema, extractionSchema } from "./schemas";
import { criticPrompt, semanticPrompt } from "./prompts";
import { assistantPrompt, extractionPrompt, reportPrompt } from "./prompts";

const client = process.env.OPENAI_API_KEY ? new OpenAI({ apiKey: process.env.OPENAI_API_KEY, timeout: 120_000, maxRetries: 0 }) : null;
...
async function validated<T extends z.ZodType>(name: string, prompt: string, payload: unknown, schema: T): Promise<z.infer<T>> {
  return schema.parse(await structured(name, prompt, payload, z.toJSONSchema(schema)));
}
export async function extractFunctions(payload: unknown) {
  return validated("semantic_extraction", extractionPrompt, payload, extractionSchema);
}
export async function matchFunctions(payload: unknown) {
  return validated("semantic_matching", semanticPrompt, payload, decisionSchema);
}
export async function critique(payload: unknown) {
  return validated("evidence_critic", criticPrompt, payload, criticSchema);
}
export async function embed(texts: string[]): Promise<number[][]> {
  if (!client || !texts.length) return [];
  const vectors: number[][] = [];
  for (let index = 0; index < texts.length; index += 64) {
    const result = await withRetry(() => client.embeddings.create({ model: process.env.OPENAI_EMBEDDING_MODEL || "text-embedding-3-small", input: texts.slice(index, index + 64).map((text) => text || "Не определено") }));
    vectors.push(...result.data.sort((a, b) => a.index - b.index).map((item) => item.embedding));
  }
  return vectors;
}
